...
class Decoder (nn.Module):
    ...
    def forward(self, input):
        ...
        # print('Forward - output: {}'.format(output.shape))
        # print('Forward - Min {} Max {}'.format(torch.min(output), torch.max(output)))
        out = interpolate(output, size=(512, 1024), mode="bilinear", align_corners=True)
        # print('Forward - output: {}'.format(out.shape))
        # print('Forward - Min {} Max {}'.format(torch.min(out), torch.max(out)))
