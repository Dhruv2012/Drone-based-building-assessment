...
def Get3Dfrom2D(List2D, K, R, t, depth_map, d=1.75):
	...
	for p in List2D:
		...
		p3D = cam_world + depth_map[p[1], p[0]] * unit_vector
...
def Get2DCoordsFromSegMask(img):
	...
	imgray = cv2.resize(imgray, (1920, 1080))
...
def ReconstructEntire3DStructure(resultsPath, datasetPath, depthMapPath):
	"""
	...
	for fileName in os.listdir(datasetPath):
		...
		if fileName.endswith(".png"):
			...
			depthMapFile = os.path.join(depthMapPath, imageName+".geometric.bin")
			if not os.path.exists(depthMapFile):
				continue
			depth_map = readDepth(depthMapFile)
...
def getPointCloud(rgbFile, depthFile):
	...
	print('rgbshape: {} depthshape: {}'.format(rgb.size, depth.shape))
